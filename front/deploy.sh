...
echo ""
ssh -i "$SSH_KEY" "$REMOTE_USER@$REMOTE_HOST" "cd $REMOTE_PATH && docker load < pazini-app.tar.gz && rm -f pazini-app.tar.gz && docker rm -f pazini-app || true && docker compose up -d --force-recreate && docker image prune -f"
