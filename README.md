...
**Login da aplicação web** (`front/.env`): defina `JWT_SECRET` (mínimo **32 caracteres**) para assinar o cookie de sessão. As rotas **`/api/*`** só respondem com sessão válida (mesmo cookie `httpOnly`); sem autenticação, retornam **401** em JSON. Os **uploads** (`/api/upload/*`) também validam sessão no handler e impõem **tamanho máximo** e **tipo real do arquivo** (assinatura binária), não apenas o MIME enviado pelo navegador. Os usuários ficam no SurrealDB (`portal_user`). Para criar o primeiro usuário a partir do `.env`, rode em `front/`: `bun run seed:portal-user` (usa `PAZINI_LOGIN_EMAIL` / `PAZINI_LOGIN_PASSWORD`).

**Convite por e-mail (novos usuários):** em **Configurações → Usuários**, informe só o e-mail; o sistema envia um link para **`/convite?token=…`** (válido **48 h**) para a pessoa **criar a senha**. Configure **URL pública do sistema** e **SMTP** em **Configurações da empresa** → seção **E-mail (convites de usuário)** (host, porta, usuário, senha, remetente). Exemplo Microsoft 365: `smtp.office365.com`, porta **587**, sem “SSL direto” na 465. A senha SMTP fica no SurrealDB (proteja backups). **Fallback opcional:** se esses campos estiverem vazios no banco, a app ainda lê `APP_URL` / `NEXT_PUBLIC_APP_URL` e `SMTP_*` do `.env` (útil em CI ou migração). Se o envio falhar, o cadastro do convite é desfeito e um erro é exibido.

Quem já está no portal pode **inativar/reativar** e **alterar senha** no modal da linha; a **remoção** é pelo ícone de lixeira, com confirmação e as mesmas regras de “pelo menos um usuário ativo” e de não remover a própria conta. Conta sem senha definida ainda não consegue logar na tela inicial até usar o link do convite (ou até um administrador definir senha no modal).

#### Convite / SMTP: erro `535 5.7.3 Authentication unsuccessful` (Microsoft 365)

O log do Nodemailer mostra a linha da senha como `/* secret */` (em base64) **só para não vazar a senha no terminal** — não é o valor lido do `.env`. Se aparece **`535 5.7.3 Authentication unsuccessful`**, a conexão e o STARTTLS estão ok; o servidor **recusou usuário/senha ou SMTP autenticado** para essa caixa.

1. **Confirme usuário e senha** no navegador: entre em [Outlook na Web](https://outlook.office.com/mail/) com o mesmo `SMTP_USER` e a mesma senha do `.env`. Se não entrar, corrija a senha no `.env` ou redefina no Microsoft 365.
2. **Ative SMTP autenticado na caixa** (muito comum estar desligado): Centro de administração do **Exchange** → **Destinatários** → **Caixas de correio** → selecione o usuário → **Gerenciar configurações de aplicativo de email** → **SMTP autenticado** = **ativado**. (Em alguns tenants o padrão é desativado.)
3. **Conta com MFA (autenticação multifator):** a senha normal do login costuma **não** servir no SMTP. Crie uma **senha de aplicativo** (Segurança da conta Microsoft / Microsoft 365) e use em `SMTP_PASS`, ou use uma conta de serviço sem MFA só para envio.
4. **Política do tenant:** o administrador pode bloquear “autenticação básica” para SMTP. Nesse caso é preciso liberar SMTP autenticado para essa caixa ou outro método (ex.: conector / Graph API — fora do escopo deste README).
5. **Caracteres especiais na senha:** no `.env`, use aspas se necessário, ex.: `SMTP_PASS="sua/senha/com caracteres"`.

**PowerShell (Exchange Online)** — se a UI não expuser a opção, o administrador pode rodar (exemplo):  
...
### Aba Ambientes (orçamento sem compositor)
